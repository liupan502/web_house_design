...
@Component({
  selector: 'app-bayWindow',
  templateUrl: './bayWindow.component.html',
  styleUrls: ['./bayWindow.component.css']
})
 ...
 * @desc 能实现内部数据类型的转换
 */
export class BayWindowComponent extends PropertyPanel implements OnInit {
   /**真正和前端绑定的数据 */
   public objinfo: any;
   ...
   constructor(private designService: DesignService, 
    ...
    private cdRef: ChangeDetectorRef) {
      ...
      this.propertyRange_ = [
        ...
        new PropertyRange({name: 'depth_',
        id: PropertyCompares.Property_Between,
        range: [0, 1000],
        obj: this.objinfo,
        alarm: '深度必须大于0，小于1000',
        }), 
  ...
  injectval() {
     ...
     this.data.depth_ = parseFloat(this.objinfo.depth_);
  ...
  hoopup() {
    ...
    this.objinfo.depth_ = this.getProperty('depth_');
  ...
  protected doModify(): boolean {
    ...
    if (this.data.depth_ !== this.prevdata.depth_) {
      return true;
    }
